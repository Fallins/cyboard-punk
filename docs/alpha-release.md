# CYBOARD macOS v1.0.0-alpha Packaging

CYBOARD v1.0.0-alpha is the first VRM-character alpha checkpoint.
...
Do not package an alpha if `bun run check` is red.
...
- The primary NYX stage renders the reviewed VRM runtime, respects camera lock/reset, and shows a brief closeout bubble when a session ends.
- The optional desktop NYX companion stays free of dashboard data and can show the same closeout bubble.
...
```bash
bun run bundle:alpha
```

For a local/tester alpha without an Apple Developer ID certificate, use an ad-hoc signature:
...
The `.dmg` is the normal artifact to give an alpha tester. The tester drags `CYBOARD.app` into Applications.
...
This is a separate packaging path and is not required for the current CYBOARD alpha.

## Versioning

The package, Tauri bundle, and Rust crate versions all use `1.0.0-alpha`. Do not create a tag, notarized release, or public upload until an explicit release authorization is given.
